...
robot = XArm7Robot()
# robot.arm.set_tcp_offset([0, 0, 160, 0, 0, 0])
...
while(t < num_points):
    ...
    # set servo pos
    # ret = robot.arm.set_servo_angle_j(angles=joint_positions, speed=math.radians(50), is_radian=True)
    # print(f"ret: {ret}")

    # step ik
    # robot.arm_ik.bullet_client.stepSimulation()

    print("p before: ", robot.get_end_effector_pose())
    # arm cart controller
    #robot.arm.set_tool_position(0, 1000, 0, 0, speed=math.radians(150), is_radian=True)
    robot.arm.set_position(x=300, y=0, z = 200, roll=180, pitch=0, yaw=0, is_radian=True, wait=True)
    print("p after: ", robot.get_end_effector_pose())
